class Autocomplete {
  ...
  getMatches( text ) {
  let result = []
      for (let index = 0; index < this.input.children.length; index++) {
        if (this.input.children[index].innerText.includes(text)) {
          result.push(
            {
              text: this.input.children[index].innerText,
              value: this.input.children[index].value
            }
          )
        }
      }
